    ...
    def run(self, save) -> tuple[float, float]:
        ...
        if self.autocorr:
            if save: 
                ...
                self.ax_autocorr.set_title(f"Autocorrelation {self.region} {self.filt1} - {self.filt2} vs. {self.filty}", fontsize=22)
                self.ax_autocorr.legend(prop={'size': 16})
...
if __name__ == '__main__':  
    ...
    for comb in filt_combinations: 
        ...
        est = Estimator(
            ...
            top_fraction=1.0, max_y_err=0.3, slope_minus=0.5, 
        )
        est.run(save=True) 
